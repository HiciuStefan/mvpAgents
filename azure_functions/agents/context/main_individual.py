...
def main():
    safe_print("Initializare procesare individuala...")

    # 1. Incarcare contexte si scenarii din Supabase (la fel ca in main.py)
    ...
    # 2. Normalizare si pregatire continut (la fel ca in main.py)
    ...
    # 3. Procesare individuala a fiecarui item
    processed_items_analysis = []
    total_items = len(all_items)
    safe_print(f"Se incepe procesarea individuala a {total_items} iteme...")

    for i, item in enumerate(all_items):
        safe_print(f"\n--- Procesare item {i+1}/{total_items} ---")
        
        content_for_rag = item.get('body') or item.get('content') or item.get('text')
        if content_for_rag:
            safe_print(f"  -> Se genereaza contextul RAG...")
            rag_context = get_rag_context(content_for_rag)
        else:
            rag_context = ""
            safe_print("  -> Avertisment: Nu s-a gasit continut pentru RAG.")

        safe_print(f"  -> Se trimite item-ul la LLM pentru analiza...")
        analysis_result = get_llm_analysis_individual(user_context, rag_context, item)
        processed_items_analysis.append(analysis_result)
        safe_print(f"  -> Analiza finalizata.")

    # 4. Salvare rezultate in fisier (la fel ca in main.py)
    output_path = 'processed_results_individual.json'
    ...
    # 5. Procesare si trimitere iteme la dashboard si RAG (la fel ca in main.py)
    if processed_items_analysis:
        ...
        for item_with_analysis in processed_items_analysis:
            ...
            if not original_item or not analysis:
                safe_print("Avertisment: Un item din analiza LLM nu contine original_item sau analiza. Se ignora.")
                continue
    ...
    safe_print("Procesare individuala finalizata cu succes!")

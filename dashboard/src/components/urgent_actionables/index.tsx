...
export async function UrgentActionables() {
  ...
  // Get channel counts
  const channelCounts = latest_items.reduce(
    (acc, item) => {
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          ...
          <span className="text-[10px] uppercase tracking-wide">
            {latest_item
              ? 'Updated ' +
                formatDistanceToNow(latest_item.createdAt, { addSuffix: true })
              : 'No items found'}
          </span>
        </div>
        {/* <CardDescription>
        ...
        <CardDescription>
          <div className="flex items-center gap-4 mt-1">
            <div className="flex items-center gap-1.5">
              <div
                className={`${styles.some_rounded_thing} bg-red-100 text-red-700`}
              >
                {priorities.high}
              </div>
              <div
                className={`${styles.some_rounded_thing} bg-yellow-100 text-yellow-700`}
              >
                {priorities.medium}
              </div>
              <div
                className={`${styles.some_rounded_thing} bg-green-100 text-green-700`}
              >
                {priorities.low}
              </div>
      ...
      <CardContent className="flex flex-col pr-4 gap-2">
        ...
        <div className="flex flex-col gap-2">
          {topItems.map((item, index) => (
            <Link
              href={`/items/${item.id}?ref=business-intelligence`}
              key={index}
              className="flex items-center gap-2 text-sm p-2 rounded-md bg-zinc-100 hover:bg-[#EEEFF0] transition-background"
            >
              {/* <span>{getPriorityIcon(item.priority)}</span> */}
              <span className="size-6 text-gray-600">
                <ChannelBadgeNoLabel type={item.type} className="text-xs" />
              </span>
        ...
        <div className="flex items-center gap-4 flex-wrap mt-2">
          {Object.entries(channelCounts).map(([channel, count]) => (
            <div key={channel} className="flex items-center">
              <ChannelBadgeNoLabel
                type={channel as ProcessedItemType}
                className="text-xs"
              />
        ...
        <Button className="w-full mt-2" variant="outline" asChild>
          <Link href={ROUTES.BUSINESS_INTELLIGENCE}>View all</Link>
        </Button>
      </CardContent>
    </Card>
  );

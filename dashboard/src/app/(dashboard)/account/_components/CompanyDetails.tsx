...
export function CompanyDetails() {
  ...
  const { data: existingData, isLoading } = api.companyDetails.get.useQuery();
  ...
  useEffect(() => {
    if (existingData) {
      form.reset({
        ...
        companyDescription: existingData.companyDescription ?? '',
  ...
  const upsertMutation = api.companyDetails.upsert.useMutation({
    onSuccess: () => {
      // Invalidate and refetch company details
      void utils.companyDetails.get.invalidate();
    },
  });
  ...
  const onSubmit = async (data: CompanyDetailsData) => {
    ...
    try {
      ...
      // Form will automatically update with the new data
    } catch (error) {

...
export async function get_embeddings(
  input_text: string
): Promise<CreateEmbeddingResponse> {
  const AZURE_OPENAI_API_KEY = process.env.EMBEDDINGS_API_KEY ?? '';
  ...
  if (embeddings_result.error !== null) {
    throw new Error(
      `Failed to generate embeddings: ${embeddings_result.error.message}`
    );
  }

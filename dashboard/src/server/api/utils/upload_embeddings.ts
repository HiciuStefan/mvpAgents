...
export async function upload_embeddings(
  embeddings_response: CreateEmbeddingResponse,
  input: string
): Promise<AzureSearchResponse | null> {
  const AZURE_SEARCH_API_KEY = process.env.RAG_API_KEY ?? '';

  // Construct the search URL
  const search_url =
    'https://ais-search-swc.search.windows.net/indexes/index1751888498617/docs/index?api-version=2024-07-01';
  ...
  if (upload_result.error !== null) {
    throw new Error(
      `Failed to call Azure Search API: ${upload_result.error.message}`
    );
  }
  ...
  if (json_result.error !== null) {
    throw new Error(
      `Failed to parse Azure Search response: ${json_result.error.message}`
    );
  }

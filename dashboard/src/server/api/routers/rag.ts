...
export const rag_router = createTRPCRouter({
  get: publicProcedure.input(rag_get_schema).query(async ({ input }) => {
    ...
    const search_result = await try_catch(
      client.search('*', {
        vectorSearchOptions: {
          queries: [
            {
    ...
    if (search_result.error !== null) {
      throw new Error(
        `Failed to search documents: ${search_result.error.message}`
      );
    }
  ...
  create: publicProcedure.input(rag_post_schema).mutation(async ({ input }) => {
    ...
    if (embeddings_result.error !== null) {
      throw new Error(
        `Failed to process embeddings: ${embeddings_result.error.message}`
      );
    }
    ...
    const upload_result = await try_catch(
      upload_embeddings(embeddings_response, input.input)
    );

    if (upload_result.error !== null) {
      throw new Error(
        `Failed to upload embeddings: ${upload_result.error.message}`
      );
    }
  ...
  delete: publicProcedure.input(rag_delete_schema).mutation(async () => {
    ...
    const search_result = await try_catch(
      client.search('*', { select: [keyField], top: 1000 })
    );
    ...
    console.log(
      `Found ${deleteBatch.length} documents. Deleting in batches...`
    );
    ...
    for (let i = 0; i < deleteBatch.length; i += batchSize) {
      ...
      if (delete_result.error !== null) {
        throw new Error(
          `Failed to delete batch ${i / batchSize + 1}: ${delete_result.error.message}`
        );
      }

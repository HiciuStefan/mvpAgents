...
import {
  fetch_latest_items,
  fetch_latest_items_by_type,
  fetch_item_by_id,
} from '~/server/db/fetch_items';
import { processedItemTypeValues } from '~/server/db/schema';
import {
  dateRangeValues,
  zodDateRangeEnum,
} from '~/components/filters/date_ranges';
...
export const processed_items_router = createTRPCRouter({
  ...
  getLatestByType: publicProcedure
    .input(
      z.object({
        type: z.enum(processedItemTypeValues),
        limit: z.number().min(1).default(10),
        actionable: z.boolean().default(false),
      })
    )

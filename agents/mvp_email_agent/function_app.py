...
@app.route(route="mvp_email_agent", auth_level=func.AuthLevel.FUNCTION, methods=["POST"])
def mvp_email_agent(req: func.HttpRequest) -> func.HttpResponse:
    ...
    db_history_text = req_body.get("db_history_text", "")
    load_dotenv()
    ...
    try:
        llm = AzureChatOpenAI(
            ...
            api_key=(subscription_key) if subscription_key else None,
            api_version=version,
        ...
        logging.error(f"Error processing email text: {e}")
        return func.HttpResponse(
            json.dumps({"error": "Error processing email text."}),
            status_code=500,
            mimetype="application/json"
        )
